...
| CI green on the pushed commit | ✅ run **31635842272** on `5bff4e7` — all four jobs, zero non-success steps, **on the first attempt**: Windows full build and test 6 m 9 s, Linux platform-neutral 2 m 4 s, ARM64 cross-check 34 s, UI typecheck and build 47 s. **The Linux job is the interesting half:** the whole gate runs there over a Unix domain socket rather than a named pipe — `tests/resilience.rs`, 6 passed in 4.01 s — which is what makes the 250 ms silence bound and the backpressure figures claims about the daemon rather than about how big a Windows pipe buffer is |
