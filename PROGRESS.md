...
**Current session:** S49 — the command line moves into the daemon (not started — see §8 for the prompt that starts it)
...
| **The gates** | ✅ `cargo test --workspace` **2 068 tests, 0 failed** across 72 test binaries (2 062 when the session was first called done); `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; in `ui/`: `npx tsc -b --force`, `npm run lint`, `npm run test` (809) and `npm run build` all clean |
...
| **The fault a real node found, and the fix** | ✅ the session shipped and the owner's node — connected, reachable, answering every poll — read **`Degraded`**. The reply was read into a buffer of exactly 239 bytes, the size §6 lists; a reply is 239 bytes *at least*, because `Filler` is *transmit as zero, receivers do not test*, so vendors pad and 240 or 512 is ordinary. A `recvfrom` into a buffer smaller than the datagram **truncates on Unix and fails on Windows** — `WSAEMSGSIZE`, data discarded — so every padded reply was thrown away on the release target while CI stayed green, because CI is Linux and `MockUdpNode` truncated. Read into an **MTU** now; `UdpError::Oversized` is its own value so one unreadable datagram is dropped rather than ending the pass; and the mock models **Windows**. Proved with a probe before it was fixed — `240 into 239 -> Err raw=Some(10040)` — and held by `a_real_node_socket_meets_a_datagram_bigger_than_its_buffer` over a real socket, `a_node_that_pads_its_reply_is_still_a_node_that_answered`, and `a_node_that_pads_its_reply_is_answering_and_not_degraded` over the protocol |
| **The second fault the same node found, and the one that was really it** | ✅ the **Windows firewall**. The inbound rule for `prismd.exe` covered the *Private* profile; `Ethernet 4`, the cable to the node, is *Public*. Polls went out and every reply was dropped before the process saw it. Narrowed by measurement rather than guessed: ARP resolves the node; PowerShell with **identical bytes on the identical port** gets **8 of 8**; `prismd` gets **0 of 21** — and **0 with the output disabled too**, so the 44 Hz ArtDmx was not it either. That left the executable, and the rule list showed the gap (`prismd.exe` Private, `powershell.exe` Private *and* Public). Confirmed by the owner opening the rule: `node discovered: "SGM  1" at 2.16.10.66:6454`, one reply per poll |
| **…and the desk says it now** | ✅ `Answer::ArtNetNodes::remedy`. The daemon could see the fingerprint the whole time — polls sent, and **not one datagram back, readable or not** — and said nothing more useful than the word *Degraded*. After three unanswered polls it now names both causes, the invisible one first, and port 6454; the sentence goes away the moment anything arrives. **Words from the daemon**, never composed by a client, which is `SurfaceStatus::remedy`'s rule and its reason: the sentence a client would write is *check the node*, and the node is the one thing that is working. Tests: `polls_going_out_with_nothing_at_all_coming_back_is_worth_saying` and `a_table_that_has_heard_nothing_carries_the_advice`, plus two in `ui/src/settings/settingswindow.test.tsx` |
| **The parser, against real hardware** | ✅ the node's `ShortName` field reads `SGM  1`, a NUL, and then rubbish — a UTF-16 `iGate` bleeding into a field the specification says is ASCII. The parser stops at the terminator and answers `SGM  1`; `LongName` is `SGM A4`. That is the field-by-field check against a real device that `ARCHITECTURE_SPEC.md` §14's row existed for, and it is now ticked |
| **And the panel can now say what the daemon knows** | ✅ `ArtNetCounters` on `Answer::ArtNetNodes`, drawn as one line: polls sent, replies, unreadable, dropped. With the node reading `Degraded` and the list empty, nothing separated *nobody is out there* from *nobody was asked* from *something came back and could not be read* — three faults, three remedies, one picture. The quiet counters stay off the line. Tests: two in `ui/src/settings/settingswindow.test.tsx` and the assertion in `artnet_nodes.rs::a_configured_node_that_never_answers_never_reads_ok` |
...
### 2.44 S48 verification record

Measured on 2026-08-30. A session about one sentence that was not true of any cue
list in this project: **the output at a cue does not depend on how the operator
got there.**

| Check | Result |
|---|---|
| **The worked example, asserted on frames** | ✅ `crates/prism-engine/tests/cue_tracking.rs::walking_to_a_cue_and_jumping_to_it_produce_the_same_frames`. A seven-cue list where cue 1 puts fixture 1 at 50 %, cue 5 at 100 % and cue 3 names neither: **120 frames byte for byte identical** between walking 1→2→3 and jumping to cue 7 and then to cue 3. Snap cues, so *byte-identical frame sequence* is literal from the tick the cue is taken. Fixture 1 reads **128** on both routes, which is cue 1's value — before this session the jump read **255** |
| **…and the same claim where a fade makes the transition legitimately differ** | ✅ `a_go_from_a_jumped_to_cue_carries_on_as_if_the_list_had_been_walked`. With one-second fades the two routes arrive at cue 3 from different looks, so the *transition* differs and must: an attribute fades from where it **is**. From the tick the list is settled at cue 3, four Gos onwards — **240 frames** — are byte for byte the same show |
| **…and what the old behaviour was, stated rather than remembered** | ✅ `a_jump_backwards_restores_what_the_walk_would_have_left`: the jump reads 128 for cue 1's wash, **0** for cue 7's value (cue 3 is above it and nothing below a cue reaches it), and **255** for what cue 2 asserted above it. Three assertions, one per direction the state can be wrong in |
| **Every cue of a generated list, reached both ways** | ✅ `every_cue_reached_both_ways_gives_the_same_frame`, a `proptest` over lists of one to eight cues naming up to four of six dimmers each, some of the values cue-only. **96 cases**, every cue of every generated list walked to from the top and jumped to from the end, compared on the frame a driver would receive. The worked example is one list somebody chose; this is the rule |
| **A cue-only value is taken back, and a tracking one is not** | ✅ `a_cue_only_value_is_taken_back_when_the_list_leaves_the_cue`: cue 1 at 128, cue 2 slams it to **255** cue-only, cue 3 hands it back to **128** — to what was underneath it, not to nought, because cue-only means *undo my edit* and not *turn it off* |
| **…and one with nothing underneath it leaves the merge rather than writing a nought** | ✅ `a_cue_only_value_with_nothing_under_it_leaves_the_merge_rather_than_going_to_zero`, over a home value of 128: the released attribute reads **128** and not 0. A slot a playback does not provide falls through to whatever is below it in `docs/DMX_MERGE.md` §1's stack, which is a different output from one held at zero — and it is the whole reason the tracking table's entries carry `Option<u16>` |
| **A fade across the boundary starts where the light is** | ✅ `an_inherited_attribute_fades_from_where_it_is_and_not_from_where_it_was_asserted`. Fixture 1 asserted at 0 % in cue 1, untouched by three cues, asserted at 100 % over one second in cue 5: the first frame of the fade is **0**, no frame goes backwards, and the last is **255**. The tracking state is where a cue is *going* and never where it is coming from |
| **Editing cue 2 changes what cue 3 outputs, without cue 3 being touched** | ✅ `crates/prism-core/tests/tracking.rs::editing_cue_two_changes_what_cue_three_inherits_without_cue_three_being_touched`. Cue 3's inherited value follows the edit; `Show::cue(1, "3")` is **equal to the clone taken before it**, so nothing was rewritten. This is the claim that says the state is derived rather than stored |
| **Nothing reaches past a blocking cue** | ✅ `nothing_reaches_past_a_blocking_cue`. With cue 3 blocked, the same edit to cue 2 leaves what cue 5 inherits **byte for byte** where it was. Blocking is an **edit** and not a flag, and this is why: a flag honoured at playback time would still be computed from the cues above it, so editing cue 2 would go on changing what a blocking cue puts out — which is the one thing blocking is asked for to stop |
| **A blocking cue asserts everything, and says where the values came from** | ✅ `blocking_writes_the_inherited_values_into_the_cue`: cue 3 goes from one part to **three**, carrying 32 768, 65 535 and its own 20 000; every one of them `Track`, because a value taken back at the end is not an assertion; every one of them with **no preset link**, because an inherited value is the result of somebody else's edit and copying the link would make a later `StorePreset` rewrite a cue nobody stored into |
| **…and the mark is derived** | ✅ `the_first_cue_of_a_list_inherits_nothing_and_therefore_blocks`, and `blocking_a_cue_that_already_asserts_everything_changes_nothing` — which answers with **no operations at all** and leaves the show byte-identical, `Show::set_cue_property`'s rule and an Oops step an operator would otherwise press and watch do nothing |
| **The tick makes no allocator call on the ninth path** | ✅ `crates/prism-engine/tests/tick_allocations.rs::a_tick_resolving_the_tracking_state_makes_no_allocator_call_either`. Eight cue lists over **768 slots**, every cue touching every slot, every second cue holding its values cue-only, a `Go` and a `GotoCue` in rotation: **0 allocator calls in 1 000 ticks**, 1 000 commands drained, 0 panics. The other eight paths are unchanged and read 0 — **ten tests, ten zeroes** |
| **The table is the size of the edits, not of the grid** | ✅ `crates/prism-engine/src/cue.rs::a_tracking_table_is_the_size_of_the_edits_not_of_the_grid`: 101 cues over one slot, two of which touch it — **2 entries**, not 101. Held by slot rather than by cue, which is what makes it fit: a cue-by-slot grid over a large list and a large rig is tens of megabytes of repeats, and one entry per *change* is bounded by the number of cue parts. The allocation test asserts the other end, where every cue touches every slot and the table **is** the grid — the worst case, by construction, and the one the searches are timed over |
| **A show file written before this session opens and means what it meant** | ✅ `crates/prism-domain/src/sequence.rs::a_cue_part_written_before_tracking_reads_as_a_tracking_one`, in both codecs: a JSON part with no `tracking` key, and a whole cue as **MessagePack** — which is what a `.prism` actually keeps (S15) — read back as `Track`. The default carries the meaning the old file already had, which is the half `#[serde(default)]` does not give you |
| **A show saved and reloaded resolves to the same output** | ✅ `crates/prism-core/tests/persistence.rs` unchanged and green, and the two frozen `.prism` fixtures regenerated: a cue written today carries `tracking` and one written before does not, and both come back as the same cue list |
| **The interface says which is which** | ✅ seven tests in `describe("what a cue inherits")` (`ui/src/show/sequencesheet.test.tsx`). An inherited cell carries the **number** — `(100%)`, resting and in brackets — where it used to carry a dash; a dash still means *no cue of this list sets it*, which is a different fact and is asserted as one; a cue-only cell keeps the assertion's weight and gains a mark; a blocking row says so. The visual language is S43's, three signals deep — brighter, bold, and a bar down the right-hand edge — because a desk is read at two metres in the dark and not everybody sees colour the same way (`CLAUDE.md`) |
| **…and does not compute it** | ✅ `asks the daemon what each cue inherits rather than folding the cues itself`. Every cue of the list is in the client's mirror, so the window could fold them — and must not, for `PatchPreview`'s reason: that fold is the rule the engine resolves a `Goto` through. `Query::CueTracking` is answered by `Show::cue_tracking` and drawn by `cueviewer.tsx`, which contains no tracking rule at all |
| **…and asks once, not once per cue of a chase** | ✅ `does not ask again when a playback advances a cue` — S45's finding one window along. The dependency is the list's `cues` node (`looks.ts::cuesDocument`) and not `/sequences`, which a cue advance rewrites |
| **The command has a home in all three appliers** | ✅ `crates/prism-core/tests/command_application.rs::the_three_groups_together_are_the_whole_protocol` at **64** (63 at S45). `SetCueTracking` is in the **show** group for `ConfigureExecutor`'s reason — what a cue asserts is show content — and is refused by the session and machine appliers leaving their state byte-identical |
| **…is undoable, and an Oops puts the cue back** | ✅ `the_command_is_a_show_edit_and_an_oops_puts_it_back`: a block takes cue 3 from one part to three, an Oops restores the show **byte for byte**, and a Redo puts it back. Its journal image is `SetCueProperty`'s — that sequence and the update state — because the same two things can move |
| **The answer goes through the real decoder** | ✅ `ui/src/ipc/protocol.test.ts::reads what every cue of a list inherits`, and the field-by-field refusals beside it: a message this build could not read would look like a daemon sending nonsense and the connection would resynchronise rather than a column being drawn. The attribute is a **checked** string against the generated table, never an `as` (`CLAUDE.md`) |
| **Coverage on the new code** | ✅ **`prism-domain::sequence.rs` 100 % on every column** — regions, functions and lines — and `query.rs` 100 %; the crate **99.59 % lines**, 98.69 % regions, 99.33 % functions. **`prism-engine::cue.rs` 99.85 % lines**, 99.62 % regions, **100 % functions**; `player.rs` 99.22 %; the crate **99.27 % lines**, 99.26 % regions, 99.10 % functions. **`prism-core::show.rs` 98.51 % lines** (up from 97.34 %), `command.rs` 99.08 %, `file.rs` 99.53 %; the crate **98.45 % lines**, 97.37 % regions, 98.17 % functions. Measured with `cargo llvm-cov clean --workspace` **between** crates, per §3's caveat. Workspace total **97.98 % lines**, 97.54 % regions, 98.01 % functions over 43 990 lines — unchanged to two decimal places on a workspace 736 lines larger |
| **…and two dead branches were removed rather than covered** | ✅ S19's and S21's rule, met twice. `Show::set_cue_tracking` looked the cue up **twice** — once by playback position and once by file position — and the second refusal was unreachable because both used the same trimmed predicate; the second lookup is gone and the walk is a `filter().take(1)`. `Sequence::cue_position` was public, tested and used by nobody once that lookup went, and was deleted rather than left as surface |
| **The wire budget** | ✅ `a_generated_wire_value_fits_in_a_test_thread` green. `CueTracking` and `CueTrackingMode` are two- and three-value unit enums, `TrackedValue` is three scalars, and `SetCueTracking` is one arm added to `arb::command_group_2` — a group costs one slot and there are six of them, which is what S43 bought room with |
| **The recordings** | ✅ all six regenerated (`cargo test -p prismd --test ui_* -- --ignored`) and the diffs read **structurally** rather than as base64: **step counts unchanged** — 61 in `show-recording.json` — so no `typedLine` renumbering and none of the hard-coded step indices in `ui/src/desk/desk.test.tsx` moved. The only difference in 19 changed steps is `"tracking": "Track"` on every cue part. The two `.prism` fixtures changed for the same reason |
...
Rolled up per crate from **that** run:
...
| 2026-08-30 | S48 | **Blocking is an edit, not a mode, and the argument is this session's own rule pointed at itself.** A `block: bool` on the cue reads better and is what a desk's manual would describe. It does nothing: the values a blocking cue would assert are *derived from the cues above it*, so editing cue 2 would go on changing what a blocking cue 5 puts out — which is precisely what an operator blocks a cue to stop | **`CueTrackingMode::Block` writes the inherited values into the cue** as ordinary parts, with no preset link and all of them tracking. After it the cue names everything, nothing above it reaches past it, and the file is self-contained. What the desk *shows* as blocking is then derived — a cue that inherits nothing — so an edit that gives cue 2 a new attribute correctly stops the mark, because the cue no longer asserts everything. **One flag would have been stored and wrong; one edit and one derived reading are both right** |
| 2026-08-30 | S48 | **Cue-only is a property of the cue, not a mode on the store.** It looked like S39's fourth store mode — the plan even calls it *the second half of the vocabulary S39's store modes started* — and it is not one. What an operator is deciding is about the **cue**: the same programmer stored twice may be a tracking look in one cue and a one-off in another, and a store mode would have forced the decision at the wrong moment | `CuePart::tracking`, set for a whole cue by `Command::SetCueTracking`. A stored cue **tracks**, which is what storing a look means, and cue-only is said afterwards. `StoreMode` is untouched, so S39's three still account for every value on both sides — a fourth would have needed a fourth column in `StorePreview` and a fourth row in `docs/IPC_PROTOCOL.md`'s table for something that is not about the store at all |
| 2026-08-30 | S48 | **A cue-only value handed back with nothing under it is *not held*, and that is a different output from zero.** The tempting representation is `u16` with 0 for released, and it is wrong in the one case that matters: an attribute a playback does not provide falls through to whatever is below it in `docs/DMX_MERGE.md` §1's stack — another playback, or the home value — and one held at zero **wins its slot** at zero. On a rig whose colours rest at full (punch-list B1) the two differ by the whole output | `TrackPoint::value` is `Option<u16>`, `Entry::releasing` carries the fade out per attribute, and `a_cue_only_value_with_nothing_under_it_leaves_the_merge_rather_than_going_to_zero` asserts the difference over a home of 128. The release is §2.3's asymmetry applied **per attribute** rather than per playback: an intensity fades to home, everything else holds where it stands, and the slot is dropped when the fade is over — while `off()` keeps its own all-or-nothing rule, because a playback leaving the merge one LTP slot at a time would give them up in an order nobody chose |
| 2026-08-30 | S48 | **The command line was left alone on purpose, and that is a decision rather than an omission.** `Command::SetCueTracking` is three words a grammar could learn, and `ARCHITECTURE_SPEC.md` §4.5 makes the command line *the* interface — so the obvious finish is a `Block Sequence 1 Cue 3` line beside the chooser | **Not this session.** The grammar lives in `ui/src/desk/console.ts`, in TypeScript, and **S49 moves it into the daemon** — which is the next session in the running order and is waiting on this one precisely because S45 and S48 both added words. Teaching the TypeScript parser three verbs S49 deletes is writing them twice. Nothing is lost meanwhile: the cue sheet sends the command, and no test in this project claims every command has a line (checked, rather than assumed). The follow-up prompt in §8 names it |
| 2026-08-30 | S48 | **The rule had to be written once even though three crates need it in three shapes.** The engine wants slot indices, the core wants fixture-and-attribute pairs, and a client wants neither. Writing the fold three times is three opinions about what a cue-only value falls back to | **`prism_domain::CueTrack`**, and it reports **changes** rather than a state — which is what lets the engine build a per-slot table in one pass instead of diffing a map per cue, and what keeps compiling a long list linear in the edits. The fold itself needed no undo stack and that is worth recording: two states are carried, `tracked` (tracking parts only) and `visible` (with the current cue's overlay), so *leaving a cue* is not an operation at all — the next overlay replaces this one and everything that was only ever in the overlay falls back by construction. The obvious per-attribute undo stack is wrong the first time two cue-only cues touch one attribute in a row |
| 2026-08-30 | S46 | **The desk could see the fault the whole time and had no way to say it.** A node that was connected, reachable and answering every poll read `Degraded` for an evening. The cause was this machine's inbound firewall rule — *Private* while the lighting network was *Public* — so the polls went out and every reply was dropped before the process saw them. What made it expensive was not the firewall: it was that *polls out, nothing back* and *no node here* and *a reply we could not read* all looked like the single word `Degraded` | `Answer::ArtNetNodes::remedy`, in the daemon's **own words**, after three unanswered polls, and withdrawn the moment any datagram arrives. It names the invisible cause first because the visible one is the one an operator checks anyway. Carried as words for `SurfaceStatus::remedy`'s reason (S37): the sentence a client would compose from the counters is *check the node*, and the node is the one thing that is working. **The general rule: when a status word can mean three things, the layer that knows which one says so** — a fingerprint the daemon can see and the operator cannot is a sentence waiting to be written |
| 2026-08-29 | S46 | **An end-to-end test read a value the daemon owns without waiting for it, and said in a comment that it had waited.** `session.spec.ts` dragged a window, read `style.left` the instant the mouse came up, reloaded and compared. The last drag step is still optimistic in `canvas/drag.ts` at that moment, so the two differed by exactly one step — on a runner, on a commit that touched no canvas code, and only sometimes | `settled()`: two agreeing reads a frame apart, which is *the delta has arrived* said in the only terms a browser has. **The rule already existed** — S43 wrote it for `closeWindows` after the same shape of failure — so what this cost was not the rule but looking for the places that predate it. The re-run of the identical tree was green, which is what said *race* rather than *break*, and it is recorded rather than used as the fix |
| 2026-08-29 | S46 | **A buffer the size of the packet is a bug on one platform and not on the other, and the forgiving platform is the one CI runs.** S46 shipped an ArtPollReply read into 239 bytes — the size §6 lists. A reply is 239 bytes *at least*: `Filler` is *transmit as zero, receivers do not test*, so vendors pad, and 240 or 512 is ordinary. A `recvfrom` into a buffer smaller than the datagram **truncates on Unix and fails on Windows** (`WSAEMSGSIZE`, data discarded). The owner's real node answered every poll and read `Degraded`; every test was green, because the mock truncated and the CI that runs `prism-protocols`' suite is Linux | Read into an **MTU** (`RECV_BUFFER_BYTES`), not into the size of the packet expected. `UdpError::Oversized` is **its own value** so one unreadable datagram is dropped and the pass carries on — as `Io` it ended a pass that may have had the real reply behind it. **`MockUdpNode` now models Windows rather than Unix**, which is the general lesson: a double that models the more forgiving platform cannot fail the way the release target fails (D10). The test that would have caught it is `a_real_node_socket_meets_a_datagram_bigger_than_its_buffer` — a **real** socket, asserting what the operating system actually does |
| 2026-08-29 | S46 | **A panel that cannot say what the daemon did leaves three faults looking identical.** With the node reading `Degraded` and the discovered-node list empty, nothing on the screen separated *nobody is out there* from *nobody was asked* from *something came back and could not be read* — and the daemon had known which it was the whole time. The counters existed and stopped at the crate boundary | `ArtNetCounters` on `Answer::ArtNetNodes`, and one line in the panel: polls sent, replies, unreadable, dropped. The quiet ones stay off the line, because six zeroes a second is six things to read past. A client renders them and interprets nothing — the diagnosis is still the operator's, but now it is possible |
...
**A cue list lands in the same place twice.** A cue is an edit rather than a
state, and what it does not name is computed from the list instead of accumulated
from wherever the playback happened to be: `prism_engine::SequencePlan` carries a
tracking table, built on the core thread whenever a sequence is recompiled, and
every cue entry — a Go either way, a `Goto`, a start — resolves through it. The
tick reads it with a binary search per slot and no allocator call, which is the
ninth measured path. It is **derived and never stored**, because a file that held
the resolved state could not be corrected by editing cue 2; clients read
`Query::CueTracking`, so the fold exists exactly once. An attribute now has three
things a cue can say about it — assert and carry forward, assert and hand back,
...
- **A default that compiles is not the same as a default that means what the old
  file meant.** `#[serde(default)]` gets an older `.prism` open; it is the
  *choice* of default that decides whether it opens as the show it was.
  `CueTracking::Track` is right here because tracking is what a cue has always
  done in this project — and a session adding a field to a persisted type should
  be able to say that sentence about its own default before it writes the
  attribute.
- **A derived state belongs on the thread that owns what it is derived from, and
  everybody else asks.** The tracking table needed no new machinery to decide
  *when* to rebuild it, because it lives in `SequencePlan` and a sequence is
  already recompiled whenever a cue moves. The question *where should this live*
  answered *when is it rebuilt* for free, which is usually the sign the home is
  the right one.
- **The obvious data structure was a grid and the right one was a list of
  changes.** A cue-by-slot table is an index rather than a search, which on the
  tick is worth something — and it is `cues × slots` cells of mostly repeats,
  rebuilt on every cue edit, on a machine that may be a Raspberry Pi. One entry
  per *change* is bounded by the number of cue parts, which is the same size as
  the edits it is derived from. **When a derived table is bigger than the thing
  it is derived from, it is the wrong shape.**
- **Making the special case general is cheaper than special-casing it.** The
  fault was in `Goto` and the fix is in every cue entry: `begin()` resolves
  through the table whatever brought the playback in. It is safe *because* it is
  a no-op for the ordinary walk, and that is the thing to check before
  generalising — a Go backwards and a start became correct without a line about
  either.
- **A flag that would be computed from the very thing it is meant to stop is not
  a flag.** Blocking looked like `block: bool` on the cue and would have done
  nothing: the values it asserts come from the cues above it, so an edit above
  would still have changed what it puts out. **Ask what the flag would be
  compared against.** Here the answer was *nothing*, so it had to become an edit,
  and the mark became derived — which is better in both halves.
- **A vocabulary that nearly fits an existing one is worth resisting.** Cue-only
  read like S39's fourth `StoreMode` and is not one: what is being decided is
  about the *cue*, and the same programmer stored twice may be a look in one cue
  and a one-off in another. A store mode would have forced the decision one
  gesture too early and added a fourth column to `StorePreview` for something
  that is not about the store.
- **Absent and zero are different outputs, and a type has to be able to say so.**
  A released attribute falls through to whatever is under it in the merge; one
  held at zero wins its slot at zero. On a rig whose colours rest at full
  (punch-list B1) that is the whole difference between a dark stage and a lit
  one, so the tracking table carries `Option<u16>` and the cue sheet draws a dash
  and a bracketed number as two different readings.
- **Dead code found by reading a coverage report is deleted, not covered.** Two
  this session: a second cue lookup in `Show::set_cue_tracking` whose refusal
  could not be reached because both lookups used the same predicate, and
...
- **A test whose name says the opposite of what it asserts is worse than no
  ...
  **new** behaviour, and would have read to the next person as a test of a bug
  that is fixed. It is `a_jump_backwards_restores_what_the_walk_would_have_left`,
  which is what it checks.

Carried out of S46:
- **A single status word that can mean three things is a diagnosis nobody can
  make.** `Degraded` covered *no node here*, *the node is silent*, *the reply was
  dropped by this machine* and *the reply arrived and could not be read*. The
  daemon could tell them apart the whole time and had no field to say it in.
  Where the desk can see a fingerprint the operator cannot, that is a sentence
  waiting to be written — and it belongs in the daemon's words, because the
  obvious sentence a client would compose sends somebody to the one thing that
  is working.
- **A test whose comment states a rule it does not follow is worse than one with
  no comment.** `session.spec.ts` said the window position was *read back after
  the delta has arrived* and read it the instant the mouse came up. It passed for
  four sessions and failed on a runner, one drag step short. S43 had already
  written the rule down for `closeWindows` — **a command out and a delta back,
  D3 applies to the suite** — so this was a known shape in a place nobody had
  looked. When a suite reads a value the daemon owns, wait for it to stop moving.
- **A receive buffer is sized for the wire, not for the packet.** 239 bytes is
  what §6 lists for an ArtPollReply and it is a *minimum*; a node that pads to
  512 is an ordinary node. Reading into exactly the expected size truncates on
  Unix and **fails** on Windows, so the fault was invisible to a Linux CI and
  fatal on the release target. Read into an MTU. **And when a double models a
  platform, model the strict one** — `MockUdpNode` truncated like Unix, so no
  test could fail the way the desk failed.
