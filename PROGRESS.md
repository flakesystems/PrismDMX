...
| **The gates** | ✅ `cargo test --workspace` **2 066 tests, 0 failed** across 72 test binaries (2 062 before the padded-reply fix below); `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; in `ui/`: `npx tsc -b --force`, `npm run lint`, `npm run test` (807) and `npm run build` all clean |
