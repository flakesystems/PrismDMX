...
| CI green on the pushed commit | ✅ run **33223231681** on `94ae402` — **all five jobs on the first attempt**: Windows 12 m 35 s, Linux neutral 4 m 38 s, UI end-to-end 3 m 22 s, UI typecheck/lint/test/build 2 m 19 s, ARM64 check 53 s. The feature commit itself was run **33209652373** on `fcb884d`, 13 m 02 s, also green on the first attempt; the two red runs between them are written up below and neither was in the code that commit shipped |
...
**Workspace total: 97.47 % lines, 97.97 % regions, 97.96 % functions** — against
a target of ≥ 85 % global. Rolled up per crate from the same run:

| Crate | Lines | Regions | Functions |
|---|---|---|---|
| `prism-engine` | **99.29 %** | 99.30 % | 99.08 % |
| `prism-domain` | **98.74 %** | 99.76 % | 99.76 % |
| `prism-surface` | **98.47 %** | 99.08 % | 98.45 % |
| `prism-core` | **97.57 %** | 98.74 % | 98.26 % |
...
at 99.29 %, the programmer — which lives in `prism-core` — at 97.57 %, and the
...
running order below). It is the session that makes this an application rather
than a browser tab, and it has one loose end waiting for it by name: S37's
