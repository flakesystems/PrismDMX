...
**Workspace total: 97.47 % lines, 97.97 % regions, 97.96 % functions** (S45,
re-measured over 65 173 lines — unchanged to two decimal places on a workspace
that grew by a thousand) — against a target of ≥ 85 % global. Rolled up per crate
from the same run:
...
protocols at 97.49 %. `prism-midi` and `prism-protocols` carry the same known
shape they have carried since S36 and S10: what is uncovered is the
open-a-real-device path, which `CLAUDE.md` forbids a test from reaching.
`prism-app` is the empty Tauri crate S29 fills.
...
**Begin S46** (Art-Net node discovery — see §8 for the prompt that starts it, and
the running order below). It is independent of everything above and answers
punch-list B6: health today means *the socket took it*, and UDP always takes it,
so a configured node reads *OK* with nothing plugged in. **S29** (the Tauri shell)
is still waiting behind the UI work with one loose end named for it: S37's
autostart switch is written down, read back and reported, and **acted on by
nobody**.
...
**The next session is S46, and not S29.** The running order in
`IMPLEMENTATION_PLAN.md` puts the remaining protocol work and the UI work before
the shell. S46 is independent of everything S45 did — it is `prism-protocols`,
`prismd` and one settings panel — so it may equally run beside S48 or S49, which
are the other two S43 gave birth to.
...
§2.41 first. That does not block S46, which touches nothing S43 built except one
panel of the settings window.
...
would be measured on cannot measure it. The recipe and the control are there.

---

```
PrismDMX — Session S46: `prism-protocols` + `prismd` + `ui` — Art-Net-Node-Discovery

Projektverzeichnis: C:\Users\Milan\Prismdmx

Der Daemon hält den Zustand und treibt DMX ohne jeden Client (D2, S18); das Pult
bedient ihn ohne Oberfläche (D11, S22); die Oberfläche ist seit S23–S28, S43,
S44, S45, S35, S34, S39 und S40 ein vollständiges Pult. Seit **S33** ist die
Verkabelung eines Saals **Daten** statt Kommandozeilen-Flags: `OutputInstance`
mit `OutputKind::ArtNet { nodes, sync, ports }`, gebaut aus Kommandos, und seit
**S37** aus einem Fenster. Was ein Output *tut*, wird pro Treiber-Thread gezählt
und als `Delta::OutputHealth` gemeldet.

**Was fehlt, ist die Wahrheit über die andere Seite des Kabels.** Ein Art-Net-
Output meldet `Health::Ok`, sobald der Socket das Datagramm angenommen hat — und
UDP nimmt immer an. Ein Installateur, der in der Settings-Oberfläche *OK* liest,
während gar kein Node am Netz hängt, wird über etwas informiert, das das Programm
nicht weiß. Das ist **B6** aus der Fehlerliste des Eigentümers
(`docs/PRERELEASE_PUNCHLIST.md`), und es ist eine eigene Session, weil es einen
**Empfangspfad** braucht, den es in `prism-protocols` bis heute nicht gibt:
gesendet wird seit S9, empfangen wurde noch nie.
...
2.  IMPLEMENTATION_PLAN.md        — **die Definition von S46**. Die fünf
                                    Deliverables und die fünf Exit-Kriterien
                                    dort sind die Anforderung, wortwörtlich.
                                    Danach die Laufreihenfolge am Ende (S46 ist
                                    Nummer 14) und die Konvention
                                    „Session-Nummern sind Identität, die
                                    Reihenfolge ist der Fahrplan"
3.  crates/prism-protocols/src/artnet.rs
                                  — der `ArtDmx`-Sender, Feld für Feld aus der
                                    Spezifikation. **Hier fehlt der Empfang**
4.  crates/prism-protocols/src/udp.rs und output.rs
                                  — der Socket, die Fehlerabbildung auf
                                    `OutputHealth`, und wo *Health* heute
                                    herkommt
5.  crates/prismd/src/outputs.rs  — der Supervisor: ein Thread pro Output, ein
                                    Neustart nur bei echter Änderung, und wie
                                    ein Zähler zum Client kommt
6.  ui/src/settings/outputs.tsx   — das Panel, das die Wahrheit anzeigen soll
7.  docs/PRERELEASE_PUNCHLIST.md  — **B6** im Wortlaut, und die drei Ausgänge,
                                    die ein Eintrag haben darf
8.  PROGRESS.md                   — §2.35 (S33 — der Output-Patch), §2.39 (S37 —
                                    das Settings-Fenster), §2.42 (S45 — was
                                    gerade gebaut wurde), §7 mit allen
                                    „Carried out of"-Listen, §3 (die Zahlen, die
                                    weiter stimmen müssen), §5 (die offenen
                                    Verifikationen — zwei davon sind Art-Net)
9.  ARCHITECTURE_SPEC.md          — §7.2 (Netzwerk-Outputs), §7.0 (der Rig als
                                    Daten), §3.1 (was in den Tick darf), §14
                                    (die Hardware-Zeilen)
10. docs/IPC_PROTOCOL.md          — §5 (`OutputChange`), §6 (`OutputHealth`,
                                    `OutputsChanged`) und §5.2 (Queries — wo
                                    eine Frage hingehört, die kein Feld ist)

Aufgabe: Session S46 umsetzen.

**Die Deliverables und Exit-Kriterien in `IMPLEMENTATION_PLAN.md` unter S46 sind
die Anforderung, vollständig und ohne Ausnahme.**

## Die Entscheidungen, die diese Session treffen und begründen muss

**Erstens: wo die Entdeckung wohnt.** Ein entdeckter Node ist weder Show noch
Session — er ist eine Beobachtung über das Netz, die sich ändert, während
niemand etwas tut, und die beim Neustart weg ist. Der Rig ist
`prism_core::MachineConfig` (S33); die MIDI-Ports sind eine **Query** (S36),
weil sie sich ändern, wenn jemand einen Stecker bewegt. Welche der beiden Formen
passt, und warum — das gehört mit dem Grund nach `docs/IPC_PROTOCOL.md` und in
den Entscheidungslog in PROGRESS.md §6.

**Zweitens: was *Health* heißt, wenn es drei Werte hat.** Heute ist es ein
Boolescher Wert aus der Sicht des Sockets. Der Plan verlangt *antwortet*, *hat
nie geantwortet*, *antwortet seit 20:14 nicht mehr*. Das ist eine
Protokolländerung an einem Typ, den drei Schichten lesen — und die dritte Form
trägt eine **Zeit**, die vom Daemon kommen muss und nicht vom Client.

## Randbedingungen

- **Kein Test darf ein Gerät oder ein echtes Netz berühren.** Kein Test sendet
  Multicast oder Broadcast auf das Netz, in dem er läuft — das ist die Regel,
  die S10 aufgeschrieben hat. Ein Mock-Node ist ein Socket auf Loopback.
- **Der Tick bleibt unberührt.** Ein Empfangspfad ist ein eigener Thread, nie
  der des Ticks, und ein Socket, der sich nicht binden lässt, degradiert
  **allein** — die Regel, die S33 für jeden Treiber aufgestellt hat. Auf der
  Frame-Folge zusichern, nicht behaupten.
- **Antworten aus dem Netz sind Eingaben von außen.** `prism-surface` hat dafür
  das Muster: ein Fuzz-Harness, kein Panic, keine Allokationslawine, und eine
  kaputte Antwort wird verworfen statt geglaubt.
...
- **Ein neues Kommando braucht eine Heimat in allen drei Appliern** und in den
  ...
  hinzugefügt, S45 eines und hat es begründet.
...
- **Vor Zeitmessungen die Systemlast prüfen**, sonst sieht eine beschäftigte
  ...
  und der blanke Tick ohne Merge-Body war sechsmal schlechter — siehe §5.
...
- **Die `data-testid`s sind ein Vertrag** zwischen Oberfläche und den 43
...
- Alle Gates grün: `cargo test --workspace`, `cargo clippy --workspace
  ...
  die Playwright-Tests (heute 43)
- Die Zahlen aus S24–S28, S33, S34, S39, S40, S36, S37, S38, S43 und S45 müssen
  weiter stimmen: null Allokationen im Tick (**acht** Pfade — S48 fügt den
  neunten hinzu, nicht diese Session), null React-Commits, das
...
- **Die Markdown-Dateien werden im selben Durchgang nachgezogen wie der Code**,
  ...
  `ARCHITECTURE_SPEC.md` (§7.2, §14 — und prüfe auf **tote Querverweise**: der
  Code zitiert Abschnitte nach Nummer), `docs/IPC_PROTOCOL.md` (Draht),
  `docs/PRERELEASE_PUNCHLIST.md` (B6 abhaken)
- PROGRESS.md aktualisieren: S46-Status, ein §2.43-Verifikationsprotokoll mit
  **jeder gemessenen Zahl**, die Coverage- und Performance-Tabellen in §3, die
  Art-Net-Zeile in §5, und in §7 eine neue Liste „Carried out of S46"
- PROGRESS.md §8 mit einem neuen, kontextfreien Folge-Prompt überschreiben —
  nach der Laufreihenfolge ist das **S48** (Tracking), das S45 als Unterbau
  braucht und es jetzt hat
