...
`fetch-ofl.sh` is the same thing for everything else; neither needs anything
that is not already on the machine.

.PARAMETER Destination
Where to put it. Defaults to `profiles\fixtures\ofl\` beside this repository.
...
if (-not $Destination) { $Destination = Join-Path $root 'profiles\fixtures\ofl' }
