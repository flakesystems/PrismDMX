...
| 5 | **S34** core/engine — executor functions and the tick readback | Fills in the four buttons S26 had to draw disabled, and the cue number S26 *and* S28 had to draw as a dash. Everything that plays back is better afterwards. **Next.** Two tests are written to go red when it lands — `ui_show.rs::the_cue_index_is_still_a_dash` and the browser's *draws the cue index as absent* |
