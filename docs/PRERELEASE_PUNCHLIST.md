...
- **Ergebnis:** ➡️ **wurde S45** — der Strip ist in S43 schon ein Fenster geworden, aber *was ein Knopf tut* zu ändern heißt, das Executor-Modell anzufassen, und das ist kein Nachmittag. S45 baut den Editor — vier Knöpfe, Fader und Encoder pro Executor, pro Seite, dazu eine **eigene Zeile** *diese Kommandozeile senden*, weil jedes Pult davon unterschiedlich viele braucht (Antwort des Eigentümers, Runde 3). Zusammen mit B18, weil beide an demselben Modell ziehen.
...
- **Ergebnis:** ➡️ **wurde S45** — und das ist die tiefere Hälfte des Eintrags: Playbacks sind heute `PlaybackId::of_executor`, also startet `Go` auf Executor 1 und `Go` auf Executor 9 **zwei** Playbacks derselben Sequence, mit je eigenem Cue-Zeiger und eigener Blende. Nicht nur die Fader laufen auseinander — die Wiedergabe selbst tut es. Das zu drehen heißt, den Zustand dorthin zu legen, wo eine Sequence nur einen davon haben kann, und das geht durch Domain, Core, Engine und die Surface-Rückmeldung. S45 macht beides in einem Zug, weil ein Editor über einem Modell, das sich gleich ändert, zweimal gebaut würde.
...
- **Ergebnis:** ✅ **behoben**, und **die Lösung ist ausdrücklich ein Provisorium**. `SurfaceAction::WriteCommandLine` trägt jetzt `submit`, und das Kästchen sitzt auf der Bindung — eine Taste auf `Go Executor 1`, die danach Enter braucht, ist keine Go-Taste, und eine Taste, die `Store Cue ` zum Fertigschreiben hinlegt, ist genau richtig; beides wird gebraucht, also entscheidet die Bindung. **Wer sie ausführt, ist die unangenehme Hälfte:** der Parser der Kommandozeile liegt in der Oberfläche (`ui/src/desk/console.ts`), nicht im Daemon, also *kann* der Daemon die Zeile nicht ausführen. Er schreibt sie und zählt `Session::command_line_run` hoch; der Client mit dem **Tastaturfokus** liest die Kante, zerlegt die Zeile und schickt, was sie bedeutet — derselbe Client, der sie bei Enter ausgeführt hätte. Auf einem Schirm ist das genau richtig. Zwei fokussierte Fenster auf zwei Maschinen deckt es nicht ab, und ein doppelter Go ist der Fehler, gegen den man hier konstruiert — deshalb ist der echte Umbau als **S49** im Plan: der Parser zieht in den Daemon, und dann führt der Daemon die Zeile selbst aus. Tests: `carries the send box on the binding, both ways` (`ui/src/settings/controls.test.tsx`) und `crates/prism-surface/src/binding.rs`.
...
- **Ergebnis:** ➡️ **verschoben nach S29** (`prism-app` — Tauri shell), auf die vom Eigentümer selbst genannte Bedingung. Der Grund ist keine Bequemlichkeit: **ein Browser kann keinen Pfad auf der Maschine des Daemons benennen.** `showOpenFilePicker` gibt einer Seite ein *Handle*, nie einen Pfad — die beiden sind absichtlich nicht dasselbe —, und der Daemon braucht einen Pfad, weil der Daemon die Datei öffnet. Die Shell nimmt diese Wand weg: ein Tauri-Kommando gibt den gewählten Pfad als Zeichenkette zurück, das Settings-Fenster legt ihn in das Feld, das es schon hat, und `OpenShow` reist unverändert. Am Protokoll ändert sich nichts, und das Feld bleibt, weil das Web Remote ein Browser ist und bleibt. **Die Control Map ist die Ausnahme und ist erledigt** (B26): sie geht durch den Dateidialog des Browsers, weil eine Tastenbelegung zu der Maschine gehört, an der der Operator sitzt, und nicht zu der, die die Show fährt.
