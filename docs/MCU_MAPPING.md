...
| Jog wheel | change the value of the selected programmer parameter | Programmer | — · **which parameter that is comes from `prism_core::Programmer::bank_parameters` (S52)** — one *function*, asked by `prismd::surface::parameter_of` here and by the encoder bar there, so the bar highlights what the wheel turns. It was one *table* (`FeatureGroup::attributes`, S26) until a bank's knobs became a question about the selection: only the parameters the selected fixtures have are on it, and a fixture may have two of one |
