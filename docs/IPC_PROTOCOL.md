...
**The desktop shell is what this was built for, and S29 is when it was first read.** `prismd::lock::look` answers *is a daemon running here* by taking a **shared** lock on `prismd.guard` and letting it go again: shared, because an exclusive one is what a daemon holds and a client that took one — even for a moment — could stop a daemon starting; and the guard file is opened read-only and never created, so asking the question leaves nothing behind in a directory no daemon has used. What the shell does with the answer, including the case where the document names a daemon it cannot reach, is `ARCHITECTURE_SPEC.md` §10.3.
...
type Command =
  ...
  | { t: "PlaceFixtures"; placements: FixturePlace[] }  // S30: where they hang, one Oops
...
>
> **`SetExecutorMaster` did not change and its meaning did.** It is still one command for a fader, and since S34 the daemon routes it through the executor's own `faderFunction`: `Master` moves the master, `Speed` moves the speed master (`docs/DMX_MERGE.md` §4.1), a **crossfade** drives a manual transition and carries no show state at all, and `Empty` does nothing. `ARCHITECTURE_SPEC.md` §6 has said what a fader does is the executor's setting since S1; this is the daemon reading it.
>
> **And there are two crossfades since S51** (punch-list B36): `XFade` and `Fade`, which are two rows of `ExecutorFaderFunction` rather than a mode field beside it. **No command was added.** The one that already carries a fader movement carries both, because which of the two a fader is belongs to the show and the movement is the same gesture either way; what the daemon adds on the way to the tick is the mode, since the tick resolves nothing (`ARCHITECTURE_SPEC.md` §3.1) and holds no show. `prism_domain::CrossfadeMode` is the two-word vocabulary that crosses that boundary, and `ExecutorFaderFunction::crossfade_mode` is the one place the two vocabularies are related.
>
> **Where a crossfade fader is, the daemon does send back — since B59.** A `Master` reads a number the show holds, so a motor may be driven to it and a second handle on the same list follows the first (B18). A crossfade read **nothing at all** from S51 until the beta, on the argument that a desk writing a position back would undo a gesture still happening; what that left was two clients each drawing their own crossfade. A crossfade move now writes `/sequences/<id>/crossfadePosition` as a `ShowPatch` — a position a hand put there, never a reading — without lighting the Save lamp, and a position the list already holds sends nothing. That is `ExecutorFaderFunction::desk_may_move_it`, asked by `prismd::surface` before any position is written and by `ui/src/desk/session.ts` before any is drawn, so the desk and the screen cannot disagree about it. `ARCHITECTURE_SPEC.md` §4.2 has the argument.
...
> **One command stops the daemon, and it is the fourth kind** *(S29)*. `Shutdown` carries nothing and changes no state: it is neither the show's, the session's nor the machine's, so it has no applier, produces no delta and puts nothing on the Oops journal. `prismd`'s own handler reads it before anything is routed and ends the run loop, which then takes `ARCHITECTURE_SPEC.md` §10.3's shutdown path exactly as Ctrl-C does — clients told first, the configured blackout-or-hold published as a frame, the outputs given time to send it, and only then the drivers stopped.
>
> It exists because §10.3 has named a **tray menu** as one of the three explicit instructions since S17 and there was nothing that could send one: a daemon spawned by a desktop shell has no console. The shell could have killed the process instead, and that is the whole argument — a kill skips every step above, so a venue's sACN receivers would hold the last look until their own timeouts ran out.
>
> **The shell sends it over the local transport**, which §2 gives the desktop shell and which is reachable only from this machine and this user. Anyone connected may send it, and that is a smaller set than it sounds: the WebSocket listener is loopback unless somebody moved it, and §2.1 makes a listener that is not loopback need a token. **No client interface offers it.** When the Web Remote arrives (S31) it must not grow a button for one — a phone in the auditorium is exactly the place a show must not be stoppable from, which is the same line §2.1 draws around the machine panel. The command line has no word for it either, and that is deliberate: `ARCHITECTURE_SPEC.md` §4.5 makes the console *the* interface and lists the exceptions, and a mistyped line that took a venue's rig off the air would be the worst of them.
>
> It answers `Ack` with an empty delta list — the daemon accepted the instruction, and what follows is a shutdown rather than a change any client could be told about. A client will then see `Reject { ShuttingDown }` on its connection, which is §8's ordinary path.
...
type Query =
  ...
  | { t: "FixtureResource"; fixtureTypeId: string; typeId: string;     // S30b
      kind: "Model" | "Wheel"; name: string; offset: number };
...
type Answer =
  ...
  | { t: "FixtureResource"; kind: "Model" | "Wheel"; name: string;     // S30b
      path: string; offset: number; total: number; data: string }  // base64
...
> **`SearchLibrary` is the variant that made the mechanism necessary** *(S44)*. `PatchPreview` could conceivably have been a client's own arithmetic, wrongly; the fixture library could not be sent at all. The desk knows some two thousand profiles, an answer has to fit in a frame, and a `LibraryEntry` is deliberately not a `FixtureType` — a key, a manufacturer, a name, a mode, a footprint, since S51 **whose profile it is**, and since S61 **which format it was read from**, which is what a menu row shows. The profile itself never leaves the daemon: `Command::EmbedFixtureType` names the one that was chosen by its key, and the daemon copies it into the show. The `limit` a client asks for is **clamped by the daemon**, because a client that asked for two thousand would otherwise get an answer no frame can carry.
>
> **`FixtureResource` — a file out of a fixture's own archive** *(S30b)*. The 3D viewer draws a GDTF device with its own models and gobo pictures, and those live in the `.gdtf` on the desk, not in the show. `fixtureTypeId` is the device's GDTF GUID (`FixturePhysical::fixtureTypeId`), which finds the archive across revisions and across desks; `typeId` is the profile key, for a profile whose GUID this desk does not know. `kind` and `name` say which file: a `Model` named `head` is `models/gltf/head.glb`, or `models/3ds/head.3ds` where the archive has no glTF; a `Wheel` picture named `15020356` is `wheels/15020356.png` (or `.jpg`, `.jpeg`, `.svg`). **`name` is a name and never a path** — one with a separator or a `..` in it is answered with nothing, because an unpacked GDTF is a directory and a name is joined to it. The answer carries the member's `path`, so a client knows the format, the file's `total` length and **one part** of it from `offset`: at most 512 KiB, as base64, so a part and the message around it stay well inside §3's 1 MiB frame; a client asks again at the offset the last part ended at. A file the desk has not got is an answer with an empty `path` and a `total` of nought, not a refusal — a profile from another desk whose archive is not here is the ordinary case. Files larger than 32 MiB are not served. The daemon holds its core only to find **where** the file is and reads it afterwards, and keeps the last eight files read, so the parts of one model do not read the archive eight times over. `ui_viewer.rs` records three of these questions and their answers — a picture served, a picture the archive lacks, a name that tries to leave the archive — and `ui/src/viewer/resources.test.ts` holds the client to those bytes.

> **`BrowseLibrary` lists the library a fixture at a time and a page at a time** *(S57, punch-list B60)*. The owner's first and third points: a fixture with four modes was four rows, and the list stopped at sixty, so a fixture past them could only be reached by typing its name. A `LibraryFixture` is a manufacturer, a name, whose it is, **whether it came out of a GDTF file** (S61), and its **modes** — each a `LibraryMode` with the profile key, the mode's name, the footprint, whether the profile has an intensity of its own (which decides whether the patch form offers the desk's dimmer) and, since S61, **how many beams its device has**. The last two are S61's, both `#[serde(default)]`, so a recording made before it reads as a library with no GDTF and no beams in it — which is what it was. They are what the patch window's *Format* column and its physical line say, and the beam count is on the **mode** because a GDTF file may describe several devices and say per mode which one a mode drives. A fixture matches when any of its modes does, by `SearchLibrary`'s rule; an empty `text` lists the library in the order it was read. `offset` is how many matching fixtures to skip, `limit` is clamped like a search's, and `matched` says whether there is another page. **The key a show embeds is still per mode**, so nothing about a stored show changed. `FixtureOfMode` answers the fixture one key belongs to, so a patched fixture's panel opens on its modes without a search. `SearchLibrary` stays, answered and tested; no window asks it since S57.
>
...
> **No window asks `StorePreview` since B62.** Its caller was the Cue Viewer's store bar, and the owner took the bar away: a cue is stored from the command line, whose question — *merge, override or remove* — is `Answer::CommandLineReading::question`. The query stays, answered and tested in Rust, because that question is the natural place for its counts to appear next; an interface that asks it again needs nothing added here.
>
