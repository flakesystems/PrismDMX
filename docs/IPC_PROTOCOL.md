...
>
> **What the daemon deliberately never sends back is where the fader is.** A `Master` reads a number the show holds, so a motor may be driven to it and a second handle on the same list follows the first (B18); a crossfade reads **nothing at all** — where it stands is the operator's hand, and a desk that wrote a position back would be undoing a gesture that is still happening. That is `ExecutorFaderFunction::desk_may_move_it`, asked by `prismd::surface` before any position is written and by `ui/src/desk/session.ts` before any is drawn, so the desk and the screen cannot disagree about it. `ARCHITECTURE_SPEC.md` §4.2 has the argument.
