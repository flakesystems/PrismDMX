  ...
  return [...screen.getByTestId("executor-bar").querySelectorAll("[data-executor]")].map(
    (element) =>
      `${element.getAttribute("data-executor") ?? ""}:${
        element.querySelector('[data-testid^="name-"]')?.textContent ?? ""
      }`,
  );
}
...
describe("the executor bar", () => {
  ...
  it("draws the buttons it cannot press, and says why", () => {
    ...
    // Four presses, and not one command: resolving `Toggle` against `isActive`
    // would be this interface deciding what the show's own setting means.
    expect(commands()).toEqual([]);
  });
