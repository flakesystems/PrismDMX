...
> **GDTF import was on that list until S61, and came off it.** The reason it was
...
| **D12** | Fixture library | **The desk does not redistribute GDTF.** The installer ships the Open Fixture Library corpus (MIT) and nothing from GDTF Share; a venue gets GDTF from its own MVR, from a manufacturer's file, or under its **own** GDTF Share account | The files are the **manufacturers'**, not GDTF Share's, and GDTF Share's terms forbid commercial use of its materials without a licence. Researched with sources in `docs/FIXTURE_LIBRARY.md` §2 — settled in S61, built out in S62 |
...
- **Absolute deadlines** via `Instant` so error does not accumulate; sleep to a millisecond before the deadline, then spin for the remainder. **The sleep is taken in slices of at most 2 ms** (`prism_engine::SLEEP_SLICE`, S63) rather than in one call: an operating system that batches timers grants a sleeper slack in proportion to what it asked for, so one 21.7 ms sleep is a loose one. Measured on macOS it overshot by 3.45 ms — past the deadline the spin was meant to cover — and the grid's median jitter was 2.5 ms against a 1 ms budget; sliced, the same machine measured a median of 76 ns. This is portable code and not a macOS branch: the crate is platform-neutral by §10.1, and Windows' coarse timer and Linux's `timer_slack_ns` are the same mechanism in different sizes.
...
| Aspect | Approach |
|---|---|
| Access path | Behind an `FtdiBackend` trait. Windows: **D2XX** (`libftd2xx`, statically linked), fallback **VCP** (`serialport` with `set_break`/`clear_break`). **macOS: the VCP alone** (S63 — see the note below). Linux/RPi: **libftdi** (`rusb`) |
...
| **macOS note** (S63) | Apple ships the VCP driver, so an FT232R appears as `/dev/cu.usbserial-…` with nothing installed — the Windows *fallback* is the macOS *default*, and there is no second path. **D2XX is not used on macOS either**, and for a sharper reason than on Linux: FTDI's library can only claim the device once Apple's own driver has been unloaded, which is a kernel extension a school cannot be asked to touch and which takes the port away from every other program while it is out. The cost is the one this table already names — the latency timer is unreachable, so the 16 ms default stands and the driver's own transmission-time arithmetic is what keeps the break out of the frame. macOS publishes **two** nodes per port, `/dev/cu.…` (callout) and `/dev/tty.…` (dial-in); opening the dial-in one **blocks until a carrier this adapter never asserts**, so `prism_protocols::vcp` filters it out before a caller can see it |
...
│  └─ fixtures/              # the fixture library: GDTF since S61, INSTALLED at
...
- **CI keeps the door open.** Windows: the full build, all tests and the installer, in `release.yml`. Linux ARM64: cross-compile check plus the platform-neutral tests, no hardware tests. **macOS: the full pass, the bundle and the signature, also in `release.yml`** — added by **S63**, and deliberately *not* in `ci.yml`: a macOS runner minute is billed at ten times a Linux one where Windows is twice, so the cost rule of 2026-09-18 applies to it more strongly rather than less. `docs/manual/apple_developer_signing.md` §9.3 says it again where somebody adding a job would be reading.
- **The ARM64 cross-check does not compile `prism-app`, and that exclusion is this rule rather than an exception to it** *(S29)*. `cargo check --workspace --exclude prism-app --target aarch64-unknown-linux-gnu` is the command. The job exists to catch platform code leaking into a crate that is supposed to be neutral; the shell is the crate it is supposed to leak *into*, it is the only one of the four exceptions whose platform code is the whole purpose of it, and building it needs a webview toolkit this job has no business installing. Everything below it is still checked on that target, which is the claim the job actually makes. What the shell *is* built by is the Windows job, all the way to the installer — because an installer that only ever builds on one person's machine is a file rather than a release. **Since 2026-09-18 that job runs in `release.yml`** — on a release tag and by hand — and no longer on every commit, for cost: a push runs Linux only, and the developer runs the full suite locally before it (§12).
- **Windows and macOS write an autostart entry; Linux does not, and the reason is the same one it always was** *(S29, amended by S63)*. §10.3's table names a systemd user unit and a `LaunchAgent` beside the `HKCU\…\Run` value. S29 implemented the registry value alone and argued for the omission this way: *no job compiles `prism-app` on Linux or macOS, so an autostart written today would be code nothing ever builds, which is the rot this section exists to prevent.*

  **S63 changed the premise on one of the two platforms and then wrote the code.** The `macos` job in `release.yml` compiles, lints, documents and tests `prism-app` on a Mac, so `prism_app::autostart::macos` is built and exercised rather than merely present — which is what the argument asked for. Linux still compiles no shell and still gets `None`, correctly. The order matters and is the rule for the next one: **the job first, the platform code second.** Everything above the `Entry` seam — the three-way comparison, the report a settings panel reads, the command line an entry carries — was already one code path on every target, which is why the macOS half was one implementation of one trait with its tests already written, exactly as S29 predicted.
...
| **Opt-in autostart** | `HKCU\…\Run` (user scope) — **built**, S29 | systemd **user** unit — not built | `~/Library/LaunchAgents` **LaunchAgent** — **built**, S63 | none |
...
**Where these run** *(2026-09-18)*. Every row above runs **locally, in full**, before
a push — that is where a change is verified. GitHub Actions repeats only what
Linux can answer on every push (`ci.yml`), and the complete pass — Windows, the
shell, the installer — only for a release (`release.yml`, or by hand with
`workflow_dispatch`). The reason is cost: Windows runner minutes are billed at
twice the Linux rate, and the Windows jobs had been running twenty minutes a push.
The hardware rows (§14) are unchanged: they never ran in CI.
